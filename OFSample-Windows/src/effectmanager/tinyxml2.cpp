...
void XMLNode::Print(XMLPrinter* streamer)
{
...
int XMLElement::InsertEndSubElements( long &counter ,XMLElement * newElement )
{
   XMLElement  * Slibenode;
//   XMLElement  * subSrcSlibenode;
   XMLDocument * doc;

   //迭代计数器
   counter ++;

   for(Slibenode = newElement; Slibenode ; Slibenode = Slibenode->NextSiblingElement())
   {//同级别节点的迁移
      doc = this->GetDocument();
      XMLNode * tmpnode;
      XMLElement * tmpelement;
      tmpnode = this->InsertEndChild(doc->NewElement(Slibenode->Name()));
      tmpelement = tmpnode->ToElement();
      //添加所有的属性

      const XMLAttribute *attribute = Slibenode->FirstAttribute();

      //添加所有的属性
      for(; attribute;attribute= attribute->Next())
      {
         //添加所有属性
          tmpelement->SetAttribute(attribute->Name(),attribute->Value());
      }

      if(0 == Slibenode->FirstChildElement())
      {//到达末尾，如果有textnode,添加text内容,否则直接返回
          if(Slibenode->GetText())
          {
            tmpelement->InsertFirstChild(doc->NewText(Slibenode->GetText()));
          }
          //判断是否是第一个节点
          //判断该节点的父节点是否存在
          if(counter != 1)
      ...
      //迭代进入下一级别
      tmpelement->InsertEndSubElements(counter,Slibenode->FirstChildElement());
      if(counter == 1)
      {//第一级别，不需要继续迭代同级别节点
            break ;
      }
...
/**
 *  功能：从当前节点子节点末端插入一个节点极其所有的子节点内容
 *  参数：需要插入的节点
 *  返回值：NULL失败
 *  附注： 只能插入节点以及其对应属性和文本
 */

int XMLElement::InsertEndSubElements( XMLElement * newElement )
{
   long counter = 0;
   int status = 0;
   if(newElement)
    status = (InsertEndSubElements(counter,newElement));
   return status;
}
/**
 *  功能：从当前节点子节点末尾插入一个XMLDocument内容
 *  参数：需要插入的节点
 *  返回值：NULL失败
 *  附注： 只能插入节点以及其对应属性和文本，其他内容拒绝插入
 */
int XMLElement::InsertEndSubElements( XMLDocument * doc )
{
     //判断docment是否存在根节点
     int status =0;
     long counter = 0;
     XMLElement * firstele = doc->FirstChildElement();
     if(firstele == NULL)
     {
         return 0;
     }
     for(;firstele;firstele=firstele->NextSiblingElement())
     {
         status = InsertEndSubElements(counter,firstele);
     }
     return 1;
}

int XMLElement::InsertFirstSubElements( long &counter ,XMLElement * newElement )
{
   XMLElement  * Slibenode;
//   XMLElement  * subSrcSlibenode;
   XMLDocument * doc;
   ...
   {//同级别节点的迁移
      ...
      XMLNode * tmpnode;
      XMLElement * tmpelement;
      tmpnode = this->InsertFirstChild(doc->NewElement(Slibenode->Name()));
      tmpelement = tmpnode->ToElement();
      //添加所有的属性
      ...
      //添加节点的所有属性
      tmpelement->InsertFirstSubElements(counter,Slibenode->FirstChildElement());
      if(counter == 1)
      {//
         break;
      }
...
/**
 *  功能：从当前节点子节点最前插入一个节点极其所有的子节点内容
 *  参数：需要插入的节点
 *  返回值：NULL失败
 *  附注： 只能插入节点以及其对应属性和文本
 */

int XMLElement::InsertFirstSubElements( XMLElement * newElement )
{
  int status = 0;
  long counter = 0;
  if(newElement)
   status = InsertFirstSubElements(counter,newElement);
  return status;
}

/**
 *  功能：  从当前节点子节点最末端插入一个XMLDocument内容
 *  参数：  需要插入的节点
 *  返回值：NULL失败
 *  附注：  只能插入节点以及其对应属性和文本，其他内容拒绝插入
 */
int XMLElement::InsertFirstSubElements( XMLDocument * doc )
{
     int status =0;
     long counter = 0;
     XMLElement * firstele = doc->FirstChildElement();
     ...
     for(;firstele;firstele=firstele->NextSiblingElement())
     {
         status = InsertFirstSubElements(counter,firstele);
     }
     return status;
}
...
/**
 *  描述：获取当前XML节点的所有子节点内容，并且以字符串格式回传
 *  返回值：返回的buffer必须由用户显式用delete释放
 */
char *XMLElement::GetAllSubNodesText()
{
    std::string strcontents;

    //XMLElement * pele  = doc->selectSingleNode(XPATH);
    XMLNode    * pnode = this->FirstChild();
    XMLPrinter  *printer;
    ...
    for(; NULL != pnode; pnode= pnode->NextSibling())
    {
         ...
         strcontents +=printer->CStr();

         delete printer;
    }

    char * buffer;
    buffer = new char[strcontents.size()+1];
    memset(buffer,0,strcontents.size()+1);
    strcpy(buffer,strcontents.c_str());
...
/**
    find  a child in this node.
    //     any position in curent opelement
    /      root node is opelement
    .      current node  is opelement
    ..     parent node   is opelement
    *      any name
    tag    chiled tag
    [@]    attribute
*/
XMLElement* XMLElement::selectSingleNode(const char *xpath)
{
    XMLElement *opelement;
    if(*xpath=='/')
    {
        opelement=this->GetDocument()->FirstChildElement();
        if(opelement)
        {
            if(*(xpath+1)=='/')
            {
                return __matchAnyNode(xpath+2,opelement);
            }
            else
            {
                return __matchSingleNode(xpath+1,opelement);
            }
        }
        else
        {
    ...
    {
        return __matchSingleNode(xpath,this);
    }
    else
    {
        return __matchSingleChildNode(xpath,this);
    }
}

static const char * __xpathcmp(const char *p1,const char *xpath)
{
    int len;
    len=0;
    for(len=0;len<256;len++)
    {
        if((*p1=='\0') || (*p1 == '/')  )
        {
            ...
            {
                if(len>0) return xpath;
            }
            return 0;
        }
        else
        {
            if(*p1 != *xpath) return 0;
        }
        p1++;xpath++;
    }
    return 0;
...
static int __xpathattrib(char *attrname,char *attrvalue,const char *xpath)
{
    if(*xpath=='[')
    {
        if(*(xpath+1)=='@')
...
XMLElement* XMLElement::__matchSingleNode(const char *xpath,XMLElement*ref)
{
    if(*xpath)
    {
        if(*xpath=='.')
        {
           if(*(xpath+1)=='/')
           {
               return __matchSingleChildNode(xpath+2,ref);
           }
           else if(*(xpath+1)=='.')
           {
                if(*(xpath+2))
                {
                    if(ref->parent) return __matchSingleNode(xpath+3,ref->parent->ToElement());
                    else return NULL;
                }
                else
                {
                    if(ref->parent) return  ref->parent->ToElement();
                    else return NULL;
                }
           }
           else if(*(xpath+1)=='\0')
        ...
        {
            return __matchAnyNode(xpath+1,ref);
        }
        else
        {
            const char *p,*nxpath;
            ...
            while( ref)
            {
                if(*xpath=='*')
                {
                   if(*(xpath+1)=='/')
                   {
                       pe= __matchSingleChildNode(xpath+2,ref);
                       if(pe) return pe;
                   }
                   else if(*(xpath+1)=='\0')
                   {
                ...
                if(nxpath)
                {
                    if(*nxpath)
                    {
                        char attrname[16];
                        char attrvalue[32];
                        if(__xpathattrib(attrname,attrvalue,nxpath))
                        {
                            const char *p;
                            p=ref->Attribute(attrname);
                            if(p)
                            {
                                if(strcmp(p,attrvalue)==0) return ref;
                            }
                        }
                        else
                        {
                            if((*(nxpath+1)=='.') && (*(nxpath+2)=='.')) pe= __matchSingleNode(nxpath+1,ref);
                            else pe= __matchSingleChildNode(nxpath+1,ref);
                            if(pe) return pe;
                        }
                    }
                    else
...
XMLElement* XMLElement::__matchSingleChildNode(const char *xpath,XMLElement*ref)
{
    if(*xpath=='/')
    {
        XMLElement *opelement;
        opelement=ref->FirstChildElement();
        if(opelement) return __matchAnyNode(xpath,opelement);
        return NULL;
    }
    else if(*xpath)
    {
        XMLElement *opelement;
        opelement=ref->FirstChildElement();
        if(opelement) return __matchSingleNode(xpath,opelement);
        return NULL;
    }
    else
    {
...
XMLElement* XMLElement::__matchAnyNode(const char *xpath,XMLElement*ref)
{
    XMLElement *fe;
    if(*xpath)
    {
        if(ref)
        {
            fe=__matchSingleNode(xpath,ref);
            if(fe)return fe;
        }
        while(ref)
        {
            ...
            if(opelement)
            {
                fe=__matchAnyNode(xpath,opelement);
                if(fe)return fe;
            }
            ref=ref->NextSiblingElement();
        }
...
void XMLElement::Print( XMLPrinter* streamer )
{
	XMLPrinter stdStreamer( stdout );
	if ( !streamer )
		streamer = &stdStreamer;
...
XMLElement* XMLDocument::selectSingleNode(const char *xpath)
{
    XMLElement*pe;
    pe=FirstChildElement();
    if(pe) return pe->selectSingleNode(xpath);
    else return pe;
}
