...
public:
	...
	*/
    ...
    /**
       print all the nodes and subnodes of itself to the std or memory
       the same method to the XMLDocument::Print
    **/
    void Print( XMLPrinter* streamer=0 );
	...
	*/
    XMLElement* selectSingleNode(const char *xpath);
	...
	*/
    ...
    /*
      描述：该方法将会以char * buffer方式返回该节点的所有内容
      内容可以包括文本或者所有子节点文本
      返回值：节点内容的文本块，该段内存XMLDOM不负责管理，必须由
      调用者显式用free释放
    */
	char* GetAllSubNodesText();
	/*
      描述：该方法会在当前节点下最末插入一个XML节点极其所关联的所有子节点
      返回值：TRUE,插入成功 FALSE:插入失败
      注释：只能插入节点/属性/TEXT,注释以及XML头内容不能插入
    */
    int InsertEndSubElements(XMLElement * newElement);
    /*
      描述：该方法会在当前节点下最末插入一段XML文本
      返回值：TRUE,插入成功 FALSE:插入失败
      注释：只能插入节点/属性/TEXT,注释以及XML头内容不能插入
    */
    int InsertEndSubElements(XMLDocument * doc);
    /*
      描述：该方法会在当前节点下最前插入一个XML节点极其所关联的所有子节点
      返回值：TRUE,插入成功 FALSE:插入失败
      注释：只能插入节点/属性/TEXT,注释以及XML头内容不能插入
    */
    int InsertFirstSubElements(XMLElement * newElement);
    /*
      描述：该方法会在当前节点下最前插入一段XML文本
      返回值：TRUE,插入成功 FALSE:插入失败
      注释：只能插入节点/属性/TEXT,注释以及XML头内容不能插入
    */
    int InsertFirstSubElements(XMLDocument * doc);
...
private:
    ...
    XMLElement* __matchSingleChildNode(const char *xpath,XMLElement*ref);
    XMLElement* __matchSingleNode(const char *xpath,XMLElement*ref);
    XMLElement* __matchAnyNode(const char *xpath,XMLElement*ref);
    ...
    int InsertEndSubElements( long &counter ,XMLElement * newElement );
    int InsertFirstSubElements( long &counter ,XMLElement * newElement );
};
