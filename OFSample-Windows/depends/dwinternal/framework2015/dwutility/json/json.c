...
char * json_unescape(char *text)
{
	...
	json_unescape2(text, result);
	return result;
}

int json_unescape2(char *text, char* result)
{
...
json_t *dw_json_tree_find_helper(const json_t *object, const char *labelnolevel)
{
    json_t * cursor = NULL;

    assert (object != NULL);
    assert (labelnolevel != NULL && *labelnolevel != '\0');

    cursor = (json_t*) object;
    if (cursor->type != JSON_OBJECT)
        cursor = cursor->child;
    if (!cursor)
        return NULL;
    cursor = cursor->child;
    ...
    while (cursor)
    {
        if (cursor->text && 0 == strcmp(labelnolevel, cursor->text))
            return cursor;
        cursor = cursor->next;
    }
...
json_t *dw_json_tree_find(const json_t *root, const char *label)
{
    char *start = NULL;
    char *curlabel = NULL;
    json_t *cursor = NULL;
    ...
    if (!root || !label || '\0' == *label)
        return NULL;

    start = (char*) templabel;
    curlabel = (char*) templabel;
    cursor = (json_t*) root;
    ...
    if ('/' == templabel[strlen(templabel) - 1])
        templabel[strlen(templabel) - 1] = '\0';

    while (*curlabel)
    {
        if ('/' == *curlabel) // find a level
        {
            *curlabel = '\0';
            cursor = dw_json_tree_find_helper(cursor, start);
            *curlabel = '/';
    ...
    return (cursor && cursor != root) ? cursor : NULL;
}

json_t *dw_json_tree_find_array_helper(const json_t* object, const int index)
{
    json_t * cursor = NULL;
    int count = 0;
    ...
    if (cursor->type != JSON_OBJECT)
        cursor = cursor->child;
    if (!cursor || JSON_ARRAY != cursor->type)
        return NULL;
    cursor = cursor->child;
...
json_t *dw_json_tree_find_array (const json_t *root, const char *label, const int index)
{
    json_t * cursor = dw_json_tree_find(root, label);
    if (!cursor)
        return NULL;
    return dw_json_tree_find_array_helper(cursor, index);
}

enum json_error  dw_json_tree_delete (const json_t *root, const char *label)
{
    json_t * cursor = NULL;
    ...
    cursor = dw_json_tree_find(root, label);
    if (!cursor)
        return JSON_UNKNOWN_PROBLEM;
...
enum json_error dw_json_tree_delete_array (const json_t *root, const char *label, const int index)
{
    json_t * cursor = NULL;

    if (!root || !label || '\0' == *label || index < 0)
        return JSON_UNKNOWN_PROBLEM;

    cursor = dw_json_tree_find_array(root, label, index);
    if (!cursor)
        return JSON_UNKNOWN_PROBLEM;
    json_free_value(&cursor);
...
json_t *dw_json_tree_set (const json_t *root, const char *label, const enum json_value_type type, const char *value)
{
    char *start = NULL;
    char *curlabel = NULL;
    char *jsonvalue = NULL;
    ...
    if (!root || !label)
        return NULL;
    if ((JSON_STRING == type || JSON_NUMBER == type) && !value)
        return NULL;
    ...
    while (*curlabel)
    {
        ...
        {
            *curlabel = '\0';
            cursor = dw_json_tree_find_helper(temp, start);
            if (!cursor)
            {
                cursor = json_new_string(start);
                if (JSON_OK != json_insert_child(temp, cursor))
                {
                    json_free_value(&cursor);
                    return NULL;
                }
                temp = cursor;
                cursor = json_new_object();
                if (JSON_OK != json_insert_child(temp, cursor))
                {
                    json_free_value(&cursor);
    ...
    if (start && *start)
    {
        ...
        if (JSON_ARRAY != type && JSON_OBJECT != type && JSON_NULL != type)
        {
            if (!value || '\0' == *value)
            {
                jsonvalue = malloc(1);
            ...
            else
            {
                ...
                jsonvalue = malloc(len + 1);
                strncpy(jsonvalue, value, len);
                jsonvalue[len] = '\0';
            }
        }
        ...
        if (!cursor) // insert
        {
            cursor = json_new_value(type);
            cursor->text = jsonvalue;
            if (JSON_OBJECT != temp->type)
                temp = temp->child;
            if (temp)
                error = json_insert_pair_into_object(temp, start, cursor);
        }
        else // set
        {
            cursor = cursor->child;
			...
			if (JSON_TRUE == type || JSON_FALSE == type)
			{
				cursor->type = type;
			}
            error = JSON_OK;
    ...
    if (JSON_OK == error && cursor && cursor->parent)
        return cursor->parent;
    return NULL;
}

enum json_error dw_json_tree_set_array (const json_t *root, const char *label, const enum json_value_type type, const int index, const char *value)
{
    json_t * cursor = NULL;
    json_t * curindex = NULL;
    char * jsonvalue = NULL;

    if (!root || !label || '\0' == *label || index < 0)
        return JSON_UNKNOWN_PROBLEM;

    cursor = dw_json_tree_set(root, label, JSON_ARRAY, NULL);
    if (!cursor)
        return JSON_UNKNOWN_PROBLEM;
    curindex = dw_json_tree_find_array_helper(cursor, index);

    if (JSON_STRING == type || JSON_NUMBER == type)
    {
        if (!value || '\0' == *value)
        {
            jsonvalue = malloc(1);
    ...
    if (!curindex)
    {
        ...
        if (!cursor || JSON_ARRAY != cursor->type)
            return JSON_UNKNOWN_PROBLEM;
        curindex = json_new_value(type);
        curindex->text = jsonvalue;
        curindex->parent = cursor;
        if (cursor->child)
        {
            cursor->child_end->next = curindex;
            curindex->previous = cursor->child_end;
            cursor->child_end = curindex;
        }
        else
            cursor->child = cursor->child_end = curindex;
    }
    else
    {
        if (curindex->text)
...
int dw_json_tree_count_array(const json_t *root, const char *label)
{
    json_t * cursor = NULL;
    int count = 0;
    ...
    cursor = dw_json_tree_find(root, label);
    if (!cursor || !cursor->child || cursor->child->type != JSON_ARRAY)
        return 0;
    cursor = cursor->child;
    if (!cursor->child)
        return 0;
