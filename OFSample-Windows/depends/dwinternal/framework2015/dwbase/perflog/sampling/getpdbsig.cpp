/** 
@file
@brief 从内存模块获取pdb签名信息
@version 2011-8-4 bobdeng
*/
...
const std::wstring GetModuleIndexString(void *pModBase, BOOL bPDB)
{
	...
	if (!bPDB) //time stamp and size of the image for a executable file
	{
		...
		assert(pNTHeaders);
		if (pNTHeaders)
			_snwprintf_s(sig, _countof(sig), _TRUNCATE, L"%08X%x", pNTHeaders->FileHeader.TimeDateStamp, pNTHeaders->OptionalHeader.SizeOfImage);
	}
	else // signature and age for a pdb file.
	{
		do
		{
			...
			if (CvSignature == CV_SIGNATURE_NB10)//CodeView format: NB10
			{
				...
				_snwprintf_s(sig, _countof(sig), _TRUNCATE, L"%08X%x", pCvInfo->Signature, pCvInfo->Age);
			}
			else if( CvSignature == CV_SIGNATURE_RSDS ) 
			{
				...
				_snwprintf_s(sig, _countof(sig), _TRUNCATE, 
