# 前言
该demo展示了如何在Windows端使用OrangeFilter SDK；数据源为获取外接摄像头数据；
目录结构解释：
- bin\release\effects:存放所有美颜相关特效包。
- bin\release\Assets：Demo所使用的资源图片，通过EffectTaskInfo.xml与特效包对应。
- src:存放逻辑代码。


# 概述

该demo展示了如何使用OrangeFilter SDK实现美颜、整形、滤镜、贴纸、手势等特效。特效包包含美颜、美妆、滤镜、整形、贴纸、手势、背景分割，特效包与功能的对应关系如下：
